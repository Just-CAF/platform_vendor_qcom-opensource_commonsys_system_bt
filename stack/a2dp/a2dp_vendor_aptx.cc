...
bool A2dpCodecConfigAptxSink::init() {
  ...
  if (A2DP_IsCodecEnabledInSink(BTAV_A2DP_CODEC_INDEX_SINK_APTX)){
    LOG_DEBUG(LOG_TAG, "%s: APTX is enabled in Sink", __func__);
    return true;
  } else {
    LOG_DEBUG(LOG_TAG, "%s: APTX is disabled in Sink", __func__);
    return false;
  }
}
